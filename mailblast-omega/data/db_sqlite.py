...
class Database:
    ...
    def register_internal_ip(self, ip: str, user_agent: str = ""):
        ...
        try:
            with self._get_conn() as conn:
                conn.execute("""
                    INSERT OR REPLACE INTO internal_ips (ip, user_agent, last_seen) 
    ...
    def add_account(self, provider, email, **kwargs):
        ...
        query = f"""
            ...
            ON CONFLICT(email, user_id) DO UPDATE SET {updates}
        """
    ...
    def get_global_stats(self, time_range='today', account_id=None):
        ...
        with self._get_conn() as conn:
            ...
            if time_range == 'today': # Rolling 24h
                where_clause += "AND sent_at >= datetime('now', '-1 day')"
            elif time_range == 'yesterday':
                where_clause += "AND sent_at >= datetime('now', '-2 days') AND sent_at < datetime('now', '-1 day')"
            elif time_range == '7d':
                where_clause += "AND sent_at >= datetime('now', '-7 days')"
            elif time_range == '30d':
                where_clause += "AND sent_at >= datetime('now', '-30 days')"
            elif time_range == '1y':
                where_clause += "AND sent_at >= datetime('now', '-1 year')"
    ...
    def get_dispatch_history(self, time_range='7d', account_id=None):
        ...
        with self._get_conn() as conn:
            ...
            if time_range == 'today': # Hourly for last 24h
                group_by = "strftime('%Y-%m-%d %H:00:00', sent_at)"
                where_clause += "AND sent_at >= datetime('now', '-1 day') "
            elif time_range == '7d':
                group_by = "date(sent_at)"
                where_clause += "AND sent_at >= datetime('now', '-7 days') "
    ...
    def add_to_blacklist(self, email: str, reason: str = 'manual'):
        ...
        with self._get_conn() as conn:
            conn.execute("""
                INSERT OR IGNORE INTO blacklist (email, reason, user_id)
                VALUES (?, ?, ?)
    ...
    def cleanup_expired_trash(self):
        with self._get_conn() as conn:
            ...
            for table in valid_tables:
                conn.execute(f"DELETE FROM {table} WHERE deleted_at IS NOT NULL AND deleted_at < datetime('now', '-7 days')")
            conn.commit()
